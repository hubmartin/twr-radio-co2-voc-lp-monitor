...
void humidity_tag_event_handler(twr_tag_humidity_t *self, twr_tag_humidity_event_t event, void *event_param)
{
    ...
    if (twr_tag_humidity_get_humidity_percentage(self, &value))
    {
        ...
        // VOC compensation
        float celsius;
        if (twr_tag_temperature_get_temperature_celsius(&temperature, &celsius))
        {
            twr_tag_voc_lp_set_compensation(&tag_voc_lp, &celsius, &value);
        }
    }
}
...
void application_init(void)
{
    ...
    twr_tag_voc_lp_set_update_interval(&tag_voc_lp, VOC_LP_TAG_UPDATE_INTERVAL);
